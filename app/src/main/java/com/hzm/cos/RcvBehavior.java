...
public class RcvBehavior extends CoordinatorLayout.Behavior<View> {
    ...
    public void onNestedPreScroll(CoordinatorLayout coordinatorLayout, View child, View target, int dx, int dy, int[] consumed) {
        ...
        if (delta > 0) {
            ...
            //以child的顶部滚动到getStatusDivHeight位置为判断dialog顶部是否继续滚动
    ...
    public void onNestedScroll(CoordinatorLayout coordinatorLayout, View child, View target, int dxConsumed, int dyConsumed, int dxUnconsumed, int dyUnconsumed) {
        ...
        View view = coordinatorLayout.getChildAt(1);
        //依赖的view向下滑动完成了，只有滑动child的view了
        if (child.getTranslationY() - dyUnconsumed < getStatusDivHeight()) {
    ...
    public void moveTop(boolean flag) {
        View view = coordinatorLayout.getChildAt(1);
        if (view.getTranslationY() <= getStatusDivHeight() || flag) {
            ObjectAnimator object1 = ObjectAnimator.ofFloat(view, "translationY", view.getTranslationY(), 0);
            ObjectAnimator object2 = ObjectAnimator.ofFloat(child, "translationY", child.getTranslationY(), 0);
    ...
    public void handleLastMove(boolean flag) {
        View view = coordinatorLayout.getChildAt(1);
        if(view.getTranslationY() <= getStatusDivHeight()){
            ObjectAnimator object1 = ObjectAnimator.ofFloat(view, "translationY", view.getTranslationY(), 0);
            ObjectAnimator object2 = ObjectAnimator.ofFloat(child, "translationY", child.getTranslationY(), view.getHeight());
            AnimatorSet animatorSet = new AnimatorSet();
            animatorSet.playTogether(object1,object2);
